...
const getGameByCategory = async (category_id) => {
  const res = await pool.query("SELECT * FROM games WHERE category_id = $1", [
    category_id,
  ]);
  return res.rows;
};
...
const getGameByuserId = async (user_id) => {
  const res = await pool.query("SELECT * FROM games WHERE user_id = $1", [
    user_id,
  ]);
  return res.rows;
};
...
const searchGame = async (name) => {
  const res = await pool.query("SELECT * FROM games WHERE name ILIKE $1", [
    `%${name}%`,
  ]);
  return res.rows;
};
